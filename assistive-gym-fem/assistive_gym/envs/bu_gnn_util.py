...
obs_limbs = {  # right hand: wrist
               # right forearm: wrist -> elbow
               # right upperarm: elbow -> shoulder
               # right foot: ankle
               # right shin: ankle -> knee
               # right thigh: knee -> hip
               # left hand: wrist
               # left forearm: wrist -> elbow
               # left upperarm: elbow -> shoulder
               # left foot: ankle
               # left shin: ankle -> knee
               # left thigh: knee -> hip
               # upperchest
               # head
    18: [[0], 'hand'],
...
def get_rectangular_limb_points(
    point1,
    point2,
    capsule_radius=None,
    length_points=0,
...
def get_body_points_from_obs(human_pose, target_limb_code,
   ...
   #     human_pose[i, 1] += 0.15
    ...
    for (limb, limb_info) in obs_limbs.items():
        ...
        if limb == 8:
            limb_points = get_torso_points(human_pose,
                    radius_upperchest=body_info[limb_name[0]][1],
                    radius_waist=body_info[limb_name[1]][1],
                    num_rings=limb_config[limb_name[0]][0])
        elif len(joints) == 1:
            limb_points = \
                get_circular_limb_points(human_pose[joints[0]],
                    radius=body_info[limb_name][1],
                    num_rings=limb_config[limb_name][0])
        else:
            limb_points = \
                get_rectangular_limb_points(human_pose[joints[0]],
                    human_pose[joints[1]],
                    capsule_radius=body_info[limb_name][1],
                    length_points=limb_config[limb_name][0],
                    width_points=limb_config[limb_name][1])

        num_limb_points = len(limb_points)
        is_target = (np.ones((num_limb_points, 1)) if limb
                     in all_possible_target_limbs[target_limb_code] else np.zeros((num_limb_points,
                     1)))
...
def sub_sample_point_clouds_recover(
    cloth_initial_3D_pos,
    cloth_intermediate_3D_pos,
    cloth_final_3D_pos,
    voxel_size=0.05,
    ):
    ...
    nb_vox = np.ceil((np.max(cloth_initial, axis=0)
                     - np.min(cloth_initial, axis=0)) / voxel_size)
    (non_empty_voxel_keys, inverse, nb_pts_per_voxel) = \
        np.unique(((cloth_initial - np.min(cloth_initial, axis=0))
    ...
    for (idx, vox) in enumerate(non_empty_voxel_keys):
        voxel_grid[tuple(vox)] = \
            cloth_initial[idx_pts_vox_sorted[last_seen:last_seen
                          + nb_pts_per_voxel[idx]]]
        voxel_grid_cloth_inds[tuple(vox)] = \
            idx_pts_vox_sorted[last_seen:last_seen
            + nb_pts_per_voxel[idx]]

        closest_point_to_barycenter = \
            np.linalg.norm(voxel_grid[tuple(vox)]
                           - np.mean(voxel_grid[tuple(vox)], axis=0),
                           axis=1).argmin()
        cloth_initial_subsample.append(voxel_grid[tuple(vox)][closest_point_to_barycenter])

       # ! the code that was being used to subsampled the intermediate state before
       # ! make sure to remove this line in your code when actually running the function! just wanted to include it for your reference

        # cloth_intermediate_subsample.append(voxel_grid[tuple(vox)][closest_point_to_barycenter])

       #   the correct approach to subsampling the point cloud
       #   the idea here is that we want our voxel grid filter to remove the same cloth points when subsampling the intermediate cloth state as when we subsampled the initial
       #   so we can't append the points in the voxel grid directly (those are points in the initial state) we need to find the points in the intermediate cloth state that share the same indicies

        cloth_intermediate_subsample.append(cloth_intermediate[voxel_grid_cloth_inds[tuple(vox)][closest_point_to_barycenter]])
   ...
   # print(len(cloth_initial_subsample), len(cloth_final_subsample))
    ...
    return (cloth_initial_subsample, cloth_intermediate_subsample,
            cloth_final_subsample)


def sub_sample_point_clouds(cloth_initial_3D_pos, cloth_final_3D_pos,
                            voxel_size=0.05):
    '''https://towardsdatascience.com/how-to-automate-lidar-point-cloud-processing-with-python-a027454a536c'''
...
def get_covered_status(all_body_points, cloth_state_2D):
    ...
    for body_point in all_body_points:
        is_covered = False
        if np.any(np.linalg.norm(cloth_state_2D - body_point[0:2],
                  axis=1) <= covered_threshold):
            is_covered = True
        is_target = body_point[2]  # 1 = target, 0 = nontarget, -1 = head
        covered_status.append([is_target, is_covered])

    covered_status = get_closest_t_to_nt_points(all_body_points,
            covered_status)
   ...
   # for nt_point in nt_points_2D:
    ...
    for i in range(len(all_body_points)):
        if covered_status[i, 0] == 0:
            ...
            min_dist_to_t_point = np.min(np.linalg.norm(t_points_2D
                    - nt_point[0:2], axis=1))
...
#%%
# Recovering Rewards
# Reward for covering target points that were uncovered
# Reward for keeping points that were covered still covered

# Recovering Penalties
# Penalty for uncovering points that were previously covered
# Penalty for keeping uncovered points uncovered

# Intermediate State: All target points should be uncovered
# Final State: All target points should be covered

def get_body_points_recovering_reward(all_body_points, cloth_initial_2D, cloth_intermediate_2D, cloth_final_2D, info=False):

   # Whether point was uncovered in intermediate state
    initially_covered_status = get_covered_status(all_body_points,cloth_initial_2D)
    intermediately_covered_status = get_covered_status(all_body_points,cloth_intermediate_2D)
   ...
   # penalties
    ...
    true_initial_uncovered_num = 0
    initally_uncovered_num = 0
    initally_covered_num = 0
    ...
    for i in range(len(covered_status)):
        ...
        is_intermediately_covered = intermediately_covered_status[i][1]

        if is_target != -1:
            if is_intermediately_covered:
                initally_covered_num += 1
            if not is_intermediately_covered and is_initially_covered:
                initally_uncovered_num += 1
                if is_covered:
                    covered_reward += 1 # Reward for covering points

            # Penalize for uncovering points
            if is_intermediately_covered and not is_covered:
                uncovered_penalty += 1

        # Penalize for covering the head
        if is_target == -1 and is_covered and not is_intermediately_covered:
    ...
    if initally_uncovered_num == 0:
        covered_reward = 100
        uncovered_penalty = -100 * uncovered_penalty
    else:
        covered_reward = 100 * (covered_reward / initally_uncovered_num)
        uncovered_penalty = -100 * (uncovered_penalty / initally_uncovered_num)
...
def get_body_points_uncovering_reward(all_body_points, cloth_initial_2D,
                           cloth_final_2D):
    initially_covered_status = get_covered_status(all_body_points,
            cloth_initial_2D)
    covered_status = get_covered_status(all_body_points, cloth_final_2D)
   ...
   # head_ind = len(covered_status)-1
    ...
    for i in range(len(covered_status)):
        ...
        elif is_target == -1 and is_covered \
            and not is_initially_covered:

                                                                         # head covered penalty
   ...
   # print(target_uncovered_reward, nontarget_uncovered_penalty, head_covered_penalty)
    ...
    num_head = np.count_nonzero(all_body_points[:, 2] == -1)
    target_uncovered_reward = 100 * (target_uncovered_reward
            / num_target)
    nontarget_uncovered_penalty = -100 * (nontarget_uncovered_penalty
            / num_target)
    head_covered_penalty = -200 * (head_covered_penalty / num_head)
    reward = target_uncovered_reward + nontarget_uncovered_penalty \
        + head_covered_penalty
...
def get_uncovering_reward(action, all_body_points, cloth_initial_2D,
    cloth_final_2D,
    ):
    reward_distance_btw_grasp_release = \
        (-150 if np.linalg.norm(np.array(action[0:2])
        - np.array(action[2:4])) >= 1.5 else 0)
    (body_point_reward, covered_status) = \
        get_body_points_uncovering_reward(all_body_points, cloth_initial_2D,
                            cloth_final_2D)
    reward = body_point_reward + reward_distance_btw_grasp_release
    return (reward, covered_status)


def get_recovering_reward(action, all_body_points, cloth_initial_2D, cloth_intermediate_2D, cloth_final_2D, info=False):
    (body_point_reward, covered_status) = get_body_points_recovering_reward(all_body_points, cloth_initial_2D, cloth_intermediate_2D, cloth_final_2D, info)
    reward = body_point_reward
    return (reward, covered_status)
    ...
    # unit_axis_vector = axis_vector/np.linalg.norm(axis_vector)
        ...
        for (i, p3) in enumerate(data[1]):
            ...
            if target_limb_code in [
                ...
                direction = ([-1, 1] if target_limb_code in [6, 7,
                             8] else [1, -1])
                vector = np.array([axis_vector[1], axis_vector[0]]) \
                    * np.array(direction)
            cloth_vector = p3[0:2] - midpoint

           # cloth_vector = midpoint-p3[0:2] if target_limb_code in [3,4,5,9,10,11] else p3[0:2]-midpoint

            d = np.linalg.norm(np.cross(vector, cloth_vector)) \
                / np.linalg.norm(vector)

            vector = vector / np.linalg.norm(vector)
            cloth_vector = cloth_vector / np.linalg.norm(cloth_vector)
            theta = np.arccos(np.clip(np.dot(vector, cloth_vector),
                              -1.0, 1.0))
...
def singulate_layer_height(grasp_loc, cloth, radius = 0.028):
    # get distance between grasp location and all points on the cloth
    dist = np.linalg.norm(cloth[:, 0:2] - grasp_loc, axis=1)
    # choose the 5 closest vertices
    closest_points = np.argpartition(dist, 10)[:5]
    # pick the vertex with the highest z
    highest_vertex = np.argmax(cloth[closest_points, 2])

    return closest_points[highest_vertex]

def check_height_of_effector(
    effector_pos,
    body_points,
    min_threshold,
    max_threshold,
...
def release_height_of_effector(effector_pos, body_points,
                               release_threshold):
   ...
   """
    ...
    for (p1_ind, point_1) in enumerate(cloth_initial):
        for (p2_ind, point_2) in enumerate(cloth_initial):  # want duplicate edges to capture both directions of info sharing
            if p1_ind != p2_ind and np.linalg.norm(point_1 - point_2) \
                <= threshold:  # don't consider distance between a point and itself, see if distance is within
