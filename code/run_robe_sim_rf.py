...
# 在导入 assistive_gym/keras 之前，尽量降低 TF 噪声，并启用按需增长，避免多进程 GPU 抢占
import os
...
#%%
def gnn_cma(env_name, idx, model, device, target_limb_code, iter_data_dir, graph_config, env_var, max_fevals, current_iter):
    ...
    graph = Runtime_Graph(
        root = iter_data_dir,
        description=f"iter_{current_iter}_processed",
        voxel_size=0.05, 
        edge_threshold=0.06, 
        action_to_all=True, 
...
def evaluate_dyn_model(env_name, target_limb_code, trials, model, iter_data_dir, device, num_processes, graph_config, env_variations, max_fevals, current_iter):
    all_results = []
    batches = int(math.ceil(trials/num_processes))
        ...
        with multiprocessing.Pool(processes=num_processes, maxtasksperchild=1) as pool:
            ...
            for i in range(num_processes):
                ...
                jobs.append(pool.apply_async(
                    gnn_cma,
                    args=(env_name, idx, model, device, target_limb_code, iter_data_dir, graph_config, env_variations, max_fevals, current_iter)
                ))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-processes', type=int, default=4)  # 新增：限制并发
    parser.add_argument('--trained-models-dir', type=str, default='./trained_models/GNN')
    ...
    for i in range(len(loop_data)):
        ...
        # 并发数：不超过 CPU-1，避免 50 这样极端值导致崩溃
        cpu_max = max(1, multiprocessing.cpu_count() - 1)
        num_processes = max(1, min(args.num_processes, cpu_max))
        trials = num_processes

        iterations = max(1, round(args.num_rollouts / num_processes))

        for current_iter in tqdm(range(iterations)):
